...
// At real-data scale a lane can hold tens of thousands of villages (IMMEDIATE
// ≈ 24k). Rendering every card freezes the page, so cap what is drawn per lane
// and show a count of the rest — the API already sorts by risk_score desc, so
// the visible cards are the highest-priority ones in the tier.
const MAX_CARDS_PER_LANE = 100;
              ...
              <div className="bg-phase-elevated border border-[#1E2330] border-t-0 rounded-b-[4px] p-3 min-h-[80px]">
                {count === 0 ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {items.slice(0, MAX_CARDS_PER_LANE).map((v) => (
                      <KanbanCard key={v.village_id} village={v} />
                    ))}
                    {items.length > MAX_CARDS_PER_LANE && (
                      <div className="text-center text-[11px] font-mono text-phase-text-secondary py-2">
                        + {items.length - MAX_CARDS_PER_LANE} more in this tier (top {MAX_CARDS_PER_LANE} by risk shown)
                      </div>
                    )}
                  </div>
                )}
