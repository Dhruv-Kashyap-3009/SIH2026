  ...
  try {
    ...
    const [
      ...
      prisma.village.findFirst({
        ...
        select: { prediction_timestamp: true, model_version: true },
      }),
    ]);
    ...
    res.json({
      ...
      // Model run metadata (for "last updated" / model-version UI) — taken from the
      // most recently predicted village row so the UI never shows a hardcoded date.
      predicted_at: latestRun?.prediction_timestamp?.toISOString() ?? null,
      model_version: latestRun?.model_version ?? null,
    });
  } catch (error) {
