...
function siteCapacityData(sites) {
  return sites
    .map((s) => ({
      // Truncated label may collide for same-named villages in different
      // districts — keep keyId (= site_id) unique for Recharts Cell keys.
      keyId: s.site_id,
          ...
          <BarChart data={capacityData} layout="vertical" margin={{ left: 20, right: 30, top: 5, bottom: 5 }}>
            ...
            <Bar dataKey="occupied" name="Occupied" stackId="a" radius={[0, 0, 0, 0]} barSize={22}>
              {capacityData.map((entry) => (
                <Cell key={entry.keyId} fill={capacityTierColor(entry.pct)} />
              ))}
            </Bar>
