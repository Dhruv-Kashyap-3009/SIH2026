...
// GET /api/sites — list all, with optional ?district= / ?state= filters and
// server-side pagination (?limit= ?offset=, applied after the suitability sort)
  ...
  try {
    ...
    const limit =
      typeof req.query.limit === "string" && /^\d+$/.test(req.query.limit)
        ? Math.min(parseInt(req.query.limit, 10), 10000)
        : undefined;
    const offset =
