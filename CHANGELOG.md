...
- **Fix:** `relocation_timeline` is now derived from the **canonical `susceptibility_score` with the exact zone cutoffs** (new `compute_relocation_timeline(df, score_col)` helper in `predict.py`): GREEN (< 0.4) → MONITOR · ORANGE (0.4–0.9) → MEDIUM_TERM · RED (≥ 0.9) → SHORT_TERM, or **IMMEDIATE** when RED and inside a documented GSI/EM-DAT disaster zone or a top-quintile population-density area. `predict_all()` no longer derives the tier from the historical score; `predict.py` and `refresh_predictions.py` both call the helper after the susceptibility model runs. `refresh_rainfall.py` (historical-model demo hook) keeps its own zones + timeline mutually consistent on `risk_score` with the same 0.9/0.4 bands, documented as a demo path.
- **New distribution (regenerated, zone × tier crosstab is a perfect diagonal — 0 contradictions in any direction):** IMMEDIATE **16,030** (was 24,220) · SHORT_TERM **4,099** (was 5,467) · MEDIUM_TERM **12,496** (was 157) · MONITOR **11,371** (was 14,152). IMMEDIATE+SHORT_TERM now exactly = the 20,129 canonical RED villages; the tiers sum to 43,996. 
- **Downstream re-run:** `refresh_predictions.py` → `generate_vyoma_export.py` (all states + Mizoram) → `generate_frontend_static.py` → `npm run seed`; backend restarted. Relocation plan / sites unchanged (they key off zones and `priority_level`, not the tier).
- **Tests hardened:** `test_pipeline.py` Test 14 now asserts the zone↔tier invariants directly (IMMEDIATE/SHORT_TERM ⊆ canonical RED; every RED village is SHORT_TERM/IMMEDIATE; no ORANGE village is SHORT_TERM/IMMEDIATE; ORANGE → MEDIUM_TERM; GREEN → MONITOR; urgent tiers sit at susceptibility_score ≥ 0.9) — 285 → 290 assertions. `test_readme_consistency.py` and README relocation tables/rules updated to the new tier rules.
- **Judges note:** the priority tiers are now exactly the canonical zones split by urgency, so kanban counts (IMMEDIATE/SHORT-TERM/MEDIUM-TERM/ROUTINE in the UI) reconcile with the risk-level distribution on the same page.

## [Unreleased] — Zone cutoffs tightened: RED ≥ 0.9 (was 0.7) (Sep 2026)

- **Request:** with too many villages classified RED, the canonical zone thresholds were moved from **RED ≥ 0.7 / ORANGE 0.4–0.7** to **RED ≥ 0.9 / ORANGE 0.4–0.9** (GREEN < 0.4 unchanged). Only the most extreme susceptibility scores now classify as RED; the ORANGE band widens to absorb the rest. The model and its scores are unchanged — this is purely a relabeling of the same score distribution.
- **Code:** the thresholds live in one definition replicated across the zone-assigning scripts and were updated in lockstep — `scripts/predict.py` (historical `predicted_risk_zone` + canonical `susceptibility_risk_zone` + `--village` view), `scripts/refresh_predictions.py`, `scripts/train_model.py` (`model_risk_zone`), `scripts/refresh_rainfall.py`. Ground-truth labels (`create_labels.py` / `update_labels_flood.py`) are untouched — they define the training label, not the output zones.
- **New canonical susceptibility distribution (regenerated from the actual data):** RED **20,129 (45.8%)** ← was 26,576 (60.4%) · ORANGE **12,496 (28.4%)** ← was 6,049 (13.7%) · GREEN **11,371 (25.8%)** (unchanged). Per-state table regenerated from the same column (sums to 20,129).
- **Downstream re-run (full chain):** `refresh_predictions.py` → `generate_relocation_sites.py` → `relocation_planner.py --capacity` → `generate_relocation_sites.py` → `generate_vyoma_export.py` (all states + Mizoram) → `generate_frontend_static.py` → `npm run seed`. Relocation plan now **8,431 assigned / 29,105 sources (29.0%)**, mean distance 35.3 km (was 8,701/29,687, 29.3%). Site register unchanged (10,603 canonical-GREEN sites, 406 ideal — GREEN set and carrying capacity untouched). `model_risk_zone` in `prediction_output.csv` and `ne_india_village_features.csv` re-derived with the new thresholds so no consumer sees stale 0.7-zone labels.
- **Unchanged by design:** `relocation_timeline` (IMMEDIATE 24,220 / SHORT_TERM 5,467 / MEDIUM_TERM 157 / MONITOR 14,152) keeps its own score+disaster-context rules — a village can now be ORANGE zone yet SHORT_TERM priority; the user asked only for zone cutoffs, so timeline semantics were left as-is (note for judges: timeline thresholds may want a future alignment pass). `recommended_action`, calibration, uncertainty, and carrying capacity are zone-independent and unchanged.
- **Tests/README updated to the new numbers** (`behavioral_vyoma.py`, `test_readme_consistency.py`, README zone tables, per-state table, relocation counts). `is_novel_red_zone` re-derived: 131 of 20,129 RED villages have no recorded historical event (0.7%).
